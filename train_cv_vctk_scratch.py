...
def parse_args():
    ...
    parser.add_argument("--save_steps", type=int, default=10000)#10000)
    ...
    parser.add_argument("--n_samples", type=int, default=1) #10
    parser.add_argument("--accu_steps", type=int, default=2)
    parser.add_argument("--batch_size", type=int, default=8)
    parser.add_argument("--n_workers", type=int, default=1) #8
    parser.add_argument("--preload", action="store_true")
    parser.add_argument("--comment", type=str)
    # add ckpt path to load pretrained model
    parser.add_argument("--ckpt_path",type=str,default="fragmentvc.pt")
...
def main(
    ...
    # # load ckpt
    # model_ckpt = torch.jit.load(ckpt_path,map_location="cpu")

    # # match state_dict keys 
    # model_keys = list(model.state_dict().keys())
    # model_ckpt_keys = list(model_ckpt.state_dict().keys())
    # print("len_keys",len(model_keys),len(model_ckpt_keys))

    # d = {}
    # for i in range(len(model_keys)):
    #     d[model_keys[i]] = model_ckpt.state_dict()[model_ckpt_keys[i]]
    ...
    criterion = nn.L1Loss()
    optimizer = AdamW(model.parameters(), lr=1e-6) #changed from 1e-4
